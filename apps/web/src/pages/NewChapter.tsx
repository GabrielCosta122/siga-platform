...
import { cn, createEntityId, formatDateLongBR } from '@/lib/utils'
...
function NewChapter() {
  ...
  function save(status: Chapter['status']) {
    if (!title.trim() || !description.trim()) { setError('Preencha o título e conte o que aconteceu antes de salvar.'); return }
  ...
  return <div className="bg-background px-4 py-8 sm:px-6 lg:px-8"><div className="mx-auto max-w-[1200px] space-y-8">
    ...
    <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_20rem] xl:grid-cols-[minmax(0,1fr)_22rem]"><form className={cn('space-y-8 rounded-xl p-1 transition-colors', selectedCategory.contextClass)} onSubmit={event => { event.preventDefault(); save('published') }}>
      <Card className="shadow-book-sm"><CardContent className="space-y-5 px-6 py-5 sm:px-8 sm:py-6"><div><label htmlFor="chapter-title" className="font-display text-2xl font-semibold tracking-tight">Qual é o título deste capítulo?</label><p className="mt-1 text-sm text-muted-foreground">Uma frase curta para encontrar esta lembrança no Livro da Casa.</p></div><Input id="chapter-title" value={title} onChange={event => { setTitle(event.target.value); setError('') }} maxLength={120} placeholder="Ex.: A primeira noite no apartamento" className="h-10 bg-secondary/55 px-4 text-base" />
        <div><label htmlFor="chapter-description" className="font-display text-xl font-semibold tracking-tight">O que aconteceu?</label><p className="mt-1 text-sm text-muted-foreground">Conte com suas palavras. Os detalhes dão vida à história.</p></div><div className="space-y-1.5"><Textarea id="chapter-description" value={description} onChange={event => { setDescription(event.target.value); setError('') }} maxLength={600} placeholder="Ex.: Visitamos a obra e vimos que a estrutura do 12º andar foi concluída." className="min-h-[9.5rem] resize-y border-0 bg-secondary/55 p-4 text-base leading-relaxed shadow-none focus-visible:border-primary focus-visible:bg-card focus-visible:ring-3 focus-visible:ring-ring/30" /><p className="text-right text-xs text-muted-foreground" aria-live="polite">{description.length} de 600 caracteres</p></div>{error ? <p role="alert" className="text-sm text-destructive">{error}</p> : null}
      </CardContent></Card>
      ...
      <footer className="flex flex-col gap-4 border-t pt-6 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm text-muted-foreground">Capítulo de {formatDateLongBR(date)}</p><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" onClick={() => save('draft')}><Save aria-hidden="true" /> Salvar como rascunho</Button><Button type="submit"><BookCheck aria-hidden="true" /> {existing ? 'Salvar alterações' : 'Salvar Capítulo'}</Button><Button type="button" variant="ghost" onClick={() => navigate('/livro-da-casa')}><X aria-hidden="true" /> Cancelar</Button></div></footer>
    </form><aside className="lg:sticky lg:top-20"><ReviewCard category={selectedCategory.label} description={title || description} impacts={selectedImpactIds.includes(unrelatedImpact.id) ? [unrelatedImpact.label] : selectedImpacts} /></aside></div>
  </div></div>
}
export { NewChapter }
